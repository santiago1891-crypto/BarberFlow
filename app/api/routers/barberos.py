...
async def listar_barberos(
    ...
    return await crud.barbero.get_multi(db, skip=skip, limit=limit)
...
async def obtener_barbero(barbero_id: uuid.UUID, db: AsyncSession = Depends(get_db)):
...
async def actualizar_barbero(
    barbero_id: uuid.UUID, barbero_in: BarberoUpdate, db: AsyncSession = Depends(get_db)
):
    db_obj = await crud.barbero.get(db, barbero_id)
    if db_obj is None:
        raise HTTPException(status_code=status.HTTP_404_NOT_FOUND, detail="Barbero no encontrado")
...
async def eliminar_barbero(barbero_id: uuid.UUID, db: AsyncSession = Depends(get_db)):
